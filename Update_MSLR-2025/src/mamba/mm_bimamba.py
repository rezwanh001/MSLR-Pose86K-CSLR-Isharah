...
try:
    ...
    print("Triton layernorm not found, using slow version")
    # Fallbacks
    ...
    def layer_norm_fn(x, weight, bias, residual=None, prenorm=True, residual_in_fp32=False, eps=1e-5):
        if residual is not None:
            x = x + residual
        mean = x.mean(-1, keepdim=True)
        std = x.std(-1, keepdim=True)
        return (x - mean) / (std + eps) * weight + bias, x

    def rms_norm_fn(x, weight, bias=None, residual=None, prenorm=True, residual_in_fp32=False, eps=1e-8):
        if residual is not None:
            x = x + residual
        norm = x.norm(dim=-1, keepdim=True) * (x.shape[-1] ** -0.5)
        out = x / (norm + eps) * weight
        if bias is not None:
            out = out + bias
        return out, x
...
class Mamba(nn.Module):
    ...
    def forward(self, a_hidden_states, a_inference_params=None): #, v_inference_params=None):
        """
        hidden_states: (B, L, D)
        ...
        if a_inference_params is not None: # and v_inference_params is not None:
            ...
            if a_inference_params.seqlen_offset > 0: # and v_inference_params.seqlen_offset > 0:
                ...
                a_out, _, _ = self.step(a_hidden_states, a_conv_state, a_ssm_state)
    ...
    def step(self, a_hidden_states, a_conv_state, a_ssm_state):
