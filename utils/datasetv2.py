...
class PoseDatasetV2(Dataset):
    ...
    # In normalize joints method
    def normalize_joints(self, joints):
        if np.sum(joints) == 0:
            return joints
        joints = joints - joints[0]
        min_vals = np.min(joints, axis=0)
        joints = joints - min_vals
        max_val = np.max(joints)
        if max_val > 0:
            joints = joints / max_val
        return joints * 0.5  # Match training normalization

    def __init__(self, dataset_name2, label_csv, split_type, target_enc_df, transform=None, augmentations=True, augmentations_prob=0.5, additional_joints=True, max_frames=512, mode="SI"):
        ...
        # if self.mode == "SI":
        #     with open(os.path.join("./data/public_si_dat/pose_data_isharah1000_hands_lips_body_May12.pkl"), 'rb') as f:
        #         self.pose_dict = pickle.load(f)
        # elif self.mode == "US":
        #     with open(os.path.join("./data/public_us_dat/pose_data_isharah1000_hands_lips_body_May12.pkl"), 'rb') as f:
        #         self.pose_dict = pickle.load(f)

        if self.mode == "SI" and self.split_type != "test":
            with open(os.path.join("/home/cpami-llm/CPAMI_WorkPlace/Rezwan/MSLR-2025/MSLR-Pose86K-CSLR-Isharah/data/public_si_dat/pose_data_isharah1000_hands_lips_body_May12.pkl"), 'rb') as f:
                self.pose_dict = pickle.load(f)
        elif self.mode == "US" and self.split_type != "test":
            with open(os.path.join("/home/cpami-llm/CPAMI_WorkPlace/Rezwan/MSLR-2025/MSLR-Pose86K-CSLR-Isharah/data/public_us_dat/pose_data_isharah1000_hands_lips_body_May12.pkl"), 'rb') as f:
                self.pose_dict = pickle.load(f)
        elif self.mode == "SI" and self.split_type == "test":
            with open(os.path.join("/home/cpami-llm/CPAMI_WorkPlace/Rezwan/MSLR-2025/MSLR-Pose86K-CSLR-Isharah/data/public_si_dat/pose_data_isharah1000_SI_test/pose_data_isharah1000_SI_test.pkl"), 'rb') as f:
                self.pose_dict = pickle.load(f)
        elif self.mode == "US" and self.split_type == "test":
            with open(os.path.join("/home/cpami-llm/CPAMI_WorkPlace/Rezwan/MSLR-2025/MSLR-Pose86K-CSLR-Isharah/data/public_us_dat/pose_data_isharah1000_US_test/pose_data_isharah1000_US_test.pkl"), 'rb') as f:
                self.pose_dict = pickle.load(f)
    ...
    def __getitem__(self, idx):
        ...
        # print (f"Processing file: {file_path}, Pose shape: {pose.shape}, Label: {label}")
        # Processing file: 17_0662, Pose shape: torch.Size([132, 86, 2]), Label: [364, 453, 265, 167]
